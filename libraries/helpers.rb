...
def set_attrs_from_data_bag(databag, item, secret=nil)
  attributes = get_data_bag_item(databag, item, secret)
...
def get_data_bag_item(databag, item, secret=nil)
  case ChefVault::Item.data_bag_item_type("#{databag}", "#{item}")
  when :normal
    data_bag_item(databag, item)
  when :encrypted
    data_bag_item(databag, item, secret)
  when :vault
